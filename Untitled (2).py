...
# محاسبه ماتریس اشتباهات و معیارهای ارزیابی
...
# جدا از قبلیه برای نمایش اطلاعات تمرین جدید (نمایش درخت تصمیم)
...
#تجزیه و تحلیل اطلاعات درخت
from sklearn.tree import _tree

tree_feature_names = list(X.columns)
tree_rules = export_text(dt_model, feature_names=tree_feature_names).split('\n')
tree_info = {'Node': [], 'Feature': [], 'Value': [], 'Decision': []}
def get_tree_structure(tree, node):
    if tree.feature[node] != _tree.TREE_UNDEFINED:
        name = tree_feature_names[tree.feature[node]]
        threshold = tree.threshold[node]
        decision = None
        if threshold <= X[name].max():
            decision = 'Yes'
        else:
            decision = 'No'  
        tree_info['Node'].append(node)
        tree_info['Feature'].append(name)
        tree_info['Value'].append(threshold)
        tree_info['Decision'].append(decision)
        
        get_tree_structure(tree, tree.children_left[node])
        get_tree_structure(tree, tree.children_right[node])
    else:
        decision = 'Leaf'
        tree_info['Node'].append(node)
        tree_info['Feature'].append('Leaf')
        tree_info['Value'].append('N/A')
        tree_info['Decision'].append(decision)
get_tree_structure(dt_model.tree_, 0)
...
#نمایش 'Accuracy', 'Precision', 'Recall', 'F1-Score'
